...
只要用户表达了"让本地文件和 ima 知识库之间产生关联"的意图，就加载。不用猜，下面是具体场景和说法：

**场景一：对比差异（最常见的入口）**
用户想看本地和 ima 知识库里各有什么、哪些不一致。典型说法：
- "帮我看看本地文件夹和知识库有什么区别"
- "检查一下 ima 里还缺哪些文件"
- "对比本地目录和知识库的内容"
- "查一下 ima 知识库里有没有这个文件"
...
以下情况不需要触发 ima-sync，应该用 ima-skills：
- 用户只是想在知识库里搜索内容（"帮我搜一下知识库里关于XX的笔记"）
- 用户想创建/编辑笔记或文件（"在知识库里建个新笔记"）
- 用户想查看知识库里有啥（"打开我的知识库看看"）

简单区分：**涉及本地文件夹 vs 知识库对比/同步 → ima-sync；纯知识库操作 → ima-skills。**
